...
create table Form(
	...
	foreign key(Add_no) references Addres(Add_no)				--引用地址表外键
)
...
begin
	...
	--查找出当前工作量最小的分拣员
	select @emp_no=Emp_no from Employees     
	where Emp_no in(
						select top 1 e.Emp_no+count( s.Sor_no) 统计 from Employees e left join Sort s on e.Emp_no=s.Emp_no
						where e.Dep_no = '1102'
						group by e.Emp_no
						order by 统计 
						)

	insert into Sort(Sor_no,Emp_no,For_no,Sor_time,Sor_state) values(@Sor_no,@emp_no,@for_no,getdate(),0)
end
go
...
--商品下架时，将下架商品存入Goodsbak------------------------------------------------------------------------------------------------
select * into Goodsbak from Goods
where 1=2
go

create trigger tri_delete_Goods
...
begin
	...
	begin
		...
		--获取该订单的客户地址编号
		select @Add_no = Form.Add_no from Form ,Sort
		where Sor_no = @Sor_no 

		--查找当前工作量最小的配送员
		select @emp_no=Emp_no from Employees
		where Emp_no in(
						select top 1 e.Emp_no+count( d.Sor_no) 统计 from Employees e left join Delivery d on e.Emp_no=d.Emp_no
						where e.Dep_no = '1101'
						group by e.Emp_no
						order by 统计 
						)
		--添加到配送表中，并且配送状态未0表示未配送，签收快照为0表示未签收
		insert into Delivery(Del_no,Sor_no,Emp_no,Add_no,Del_gotime,Del_gettime,Del_voucher,Del_status) values(@Del_no,@Sor_no,@emp_no,@Add_no,GETDATE(),'',0,0)
	end
...
begin
	...
	begin
		...
		begin
			...
			--删除已离职配送员的配送信息
--			delete from Delivery where Del_no = @Del_no and Emp_no = @Emp_no
			--查找当前工作量最小的配送员
			create table temp       --新建一个表存放工作量最小员工的信息
			(	
				Emp_no varchar(6),
...
--			insert into Delivery(Del_no,Sor_no,Emp_no,Add_no,Del_gotime,Del_gettime,Del_voucher,Del_status) values(@Del_no,@Sor_no,@new_emp_no,@Add_no,GETDATE(),null,@Del_voucher,@Del_status)
